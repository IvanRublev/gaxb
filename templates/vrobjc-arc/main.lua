...
function uncleanedName(v)
    -- removes underscore to make correct output in xml
    local cleanedV = v;
    for i,rn in ipairs(cleanMap) do
        if (cleanedV == "_"..rn) then
            cleanedV = rn;
        end
    end
    return cleanedV;
...
for k,v in pairs(schema.elements) do
	...
	if (schema.namespace == v.namespace) then	
		...
		gaxb_template("element_base.mm", className(v).."Base.mm", v);

		gaxb_template("element.h", className(v)..".h", v, false);
		gaxb_template("element.mm", className(v)..".mm", v, false);
	end
end
