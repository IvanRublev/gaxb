...
+ (id) readFromData:(NSData *)data withParent:(id)p AndMemoryLite:(BOOL)memLite forceDefaultNamespace:(BOOL)forceDefaultNamespace
{
	if(!data) return NULL;
	...
	if(TBXLCLASS != NULL) 
	{
		...
		if(UseTBXML && tbxml) 
		{
			...
			{
				return CreateElementWithNamespace(tbxml.rootXMLElement, NULL, [NSMutableDictionary dictionary], p, forceDefaultNamespace);
			}
		}
	}
...
+ (id) readFromData:(NSData *)data withParent:(id)p forceDefaultNamespace:(BOOL)forceDefaultNamespace
{
  return [self readFromData:data withParent:p AndMemoryLite:YES forceDefaultNamespace:forceDefaultNamespace];
}

+ (id) readFromData:(NSData *)data withParent:(id)p
{
    return [self readFromData:data withParent:p AndMemoryLite:YES forceDefaultNamespace:NO];
}
            
+ (id) readFromData:(NSData *)data forceDefaultNamespace:(BOOL)forceDefaultNamespace
{
    return [<%= FULL_NAME_CAMEL %>_XMLLoader readFromData:data withParent:NULL forceDefaultNamespace:forceDefaultNamespace];
}
...
+ (id) readFromFile:(NSString *)path forceDefaultNamespace:(BOOL)forceDefaultNamespace
{
    return [<%= FULL_NAME_CAMEL %>_XMLLoader readFromData:[NSData dataWithContentsOfFile:path options:0 error:NULL] forceDefaultNamespace:forceDefaultNamespace];
}
...
+ (id) readFromString:(NSString *)xml_string forceDefaultNamespace:(BOOL)forceDefaultNamespace
{
    return [<%= FULL_NAME_CAMEL %>_XMLLoader readFromData:[xml_string dataUsingEncoding:NSUTF8StringEncoding] forceDefaultNamespace:forceDefaultNamespace];
}
...
{
	return [self readFromData:data withParent:NULL AndMemoryLite:NO forceDefaultNamespace:NO];
}
...
static void SetValue(NSObject * object, NSObject * childObject, const char * elementName, const char * className)
{
	...
	if ([cc isSubclassOfClass:_NSStringClass])
	{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if (p != _NSObjectClass && ![p isSubclassOfClass:_NSStringClass])
					{
                                             ...
                                             BOOL forceDefaultNamespace)
{
    ...
    if (adjustedElementName[0]) {
        char upc[kMaxSelectorName];
        strlcpy(upc, adjustedElementName,sizeof(upc));
        upc[0] = toupper(upc[0]);
        adjustedElementName = upc;
    }
	...
	if (c)
	{
		...
		while (element)
		{
			...
			id childObject = CreateElementWithNamespace(element, currentNamespace, namespaceMap, object, forceDefaultNamespace);
